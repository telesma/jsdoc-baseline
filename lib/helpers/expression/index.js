        ...
        reparentItems: function(doclet, key) {
            ...
            if (_.getPath(template.config, 'tables.nestedPropertyTables')) {
                items.forEach(function(item) {
                    ...
                    if (itemMap[item.name]) {   // item can exist here only in case when a stub was added earlier in if (parentName){...} section. Replacing it with real item.
                        item.children = itemMap[item.name].children;
                    ...
                    ownName = dotIndex === -1 ? item.name : item.name.substr(dotIndex + 1);
                    parentName = dotIndex === -1 ? null : item.name.substr(0, dotIndex);

                    if (parentName) {
                        var parent = itemMap[parentName];

                        if (!parent) {  // no parent can be if "a.b.c" param is declared before "a.b" or before "a". Adding a stub that should be replaced.
